  ...
  const filteredNavigation = navigation
    .map((group) => ({
      ...
      items: group.items.filter((item) =>
        role ? item.roles.includes(role as "admin" | "staff") : false,
      ),
    }))
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      active &&
                        "bg-[#0F4C5C] text-white hover:bg-[#0F4C5C] hover:text-white",
                    )}
                  >
...
export function Sidebar({
  ...
  return (
    <aside
      className={cn(
        "fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-[#D7E0E3] lg:block",
        className,
      )}
    >
