...
export function HowItWorksSection() { 
  return ( 
    <section id="how-it-works" className="bg-[#0F4C5C] py-20 text-white sm:py-24"> 
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"> 
        <div className="mx-auto max-w-3xl text-center"> 
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#78A394]">Simple from day one</p> 
          <h2 className="mt-3 text-balance text-3xl font-bold tracking-tight sm:text-4xl">From setup to insight in three steps</h2> 
          <p className="mt-4 text-base leading-7 text-white/70 sm:text-lg">A straightforward workflow keeps every team member focused on their job.</p> 
        </div> 
        <ol className="relative mt-14 grid gap-6 lg:grid-cols-3"> 
          <div className="absolute top-12 right-[16%] left-[16%] hidden h-px bg-gradient-to-r from-transparent via-[#78A394]/70 to-transparent lg:block" aria-hidden="true" /> 
          {steps.map((step) => { 
            const Icon = step.icon; 
            return ( 
              <li key={step.number} className="relative rounded-2xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur"> 
                <div className="flex items-center justify-between"><span className="flex size-12 items-center justify-center rounded-xl bg-[#78A394]/15 text-[#78A394]"><Icon className="size-6" /></span><span className="text-3xl font-black text-white/10">{step.number}</span></div> 
