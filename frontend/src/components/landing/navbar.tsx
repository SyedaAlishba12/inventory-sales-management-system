    ...
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex h-18 max-w-7xl items-center px-4 sm:px-6 lg:px-8">
        ...
        <nav className="mx-auto hidden items-center gap-1 lg:flex" aria-label="Landing page navigation">
          {navigation.map((item) => (
            <a key={item.href} href={item.href} className="rounded-lg px-4 py-2 text-sm font-medium text-slate-600 outline-none transition hover:bg-slate-100 hover:text-slate-950 focus-visible:ring-2 focus-visible:ring-ring">
              {item.label}
            </a>
          ))}
        </nav>
      ...
      <Dialog open={mobileOpen} onOpenChange={setMobileOpen}>
        <DialogContent aria-describedby={undefined} className="top-0 right-0 left-auto h-dvh w-[min(22rem,88vw)] max-w-none translate-x-0 translate-y-0 rounded-none border-y-0 border-r-0 p-0 sm:hidden">
          ...
          <div className="flex h-full flex-col">
            <div className="flex h-18 items-center border-b px-5"><AppLogo /></div>
            <nav className="flex flex-1 flex-col gap-1 p-4" aria-label="Mobile landing page navigation">
              {navigation.map((item) => (
                <a key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className="rounded-xl px-4 py-3 text-base font-semibold text-slate-700 outline-none transition hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 focus-visible:ring-ring">
                  {item.label}
                </a>
              ))}
            </nav>
