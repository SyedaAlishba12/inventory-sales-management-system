...
function PurchaseDetailContent({ id }: { id: string }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [supplierData, productsData] = await Promise.all([
        apiClient
          .get<SupplierResponse>(
            `/api/suppliers/${purchaseData.supplier_id}`,
          )
          .catch(() => null),
      ...
      setProductMap(
        new Map(
          productsData.map((p: ProductSummary) => [
            p.id as string,
            p.name,
          ]),
        ),
  ...
  const columns: DataTableColumn<PurchaseItemResponse>[] = [
    {
      ...
      header: "Product",
      accessor: (row) =>
        productMap.get(row.product_id) || "Unknown Product",
    },
    {
      id: "qty",
      header: "Quantity",
...
export default function PurchaseDetailPage({
  ...
  return (
    <AuthGuard requireAdmin>
      <PurchaseDetailContent id={resolvedParams.id} />
    </AuthGuard>
  );
}